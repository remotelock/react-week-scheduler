...
import { addMinutes, addDays, isBefore, isEqual as isEqual$1, startOfDay, endOfDay, subDays, compareAsc, differenceInDays, differenceInMinutes, getMinutes, isSameDay, format } from 'date-fns';
import invariant from 'invariant';
import { isEqual, clamp, floor, round, range, times } from 'lodash';
import React, { useState, useEffect, useCallback, useRef, useMemo } from 'react';
import scrollIntoView from 'scroll-into-view-if-needed';
import { fromEvent, of, merge } from 'rxjs';
import { tap, mergeMap, delay, takeUntil, filter, map, startWith, distinctUntilChanged } from 'rxjs/operators';
...
function useClickAndDrag(ref) {var _useState =
  ...
  useEffect(function () {
    ...
    var style$ = box$.pipe(
    map(function (_ref) {var top = _ref.top,left = _ref.left,width = _ref.width,height = _ref.height;return {
...
{var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},_ref$passive = _ref.passive,passive = _ref$passive === void 0 ? true : _ref$passive,_ref$enabled = _ref.enabled,enabled = _ref$enabled === void 0 ? true : _ref$enabled;var _useState =
  useState({ scrollTop: 0, scrollLeft: 0 }),_useState2 = _slicedToArray(_useState, 2),position = _useState2[0],setPosition = _useState2[1];
...
{var totalHeight = _ref.totalHeight,totalWidth = _ref.totalWidth,numVerticalCells = _ref.numVerticalCells,numHorizontalCells = _ref.numHorizontalCells;
  var cellHeight = floor(totalHeight / numVerticalCells);
  var cellWidth = floor(totalWidth / numHorizontalCells);
  ...
  originDate = _ref.originDate;return (
    function (_ref2) {var startX = _ref2.startX,startY = _ref2.startY,endX = _ref2.endX,spanY = _ref2.spanY;
      ...
      map(function (i) {
        ...
        var endDate = addMinutes(startDate, toMin(spanY));

        if (isEqual$1(endDate, startOfDay(endDate))) {
...
var Cell = React.memo(function Cell(_ref)
...
{var takeSecond = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;var _dates$map =
  dates.map(function (date) {return format(date, template);}),_dates$map2 = _slicedToArray(_dates$map, 2),first = _dates$map2[0],second = _dates$map2[1];
  if (first !== second) {
    return [first, second];
  }
...
template2)
{
  ...
  if (isSameDay(start, end) && !template) {var _dropSame =
    ...
    return "".concat(format(start, 'ddd'), " ").concat(formatHour(
    start)).concat(
    firstM, " \u2013 ").concat(formatHour(end)).concat(secondM);
  }

  var formatTemplate = 'ddd h:mma';
  var startDateStr = format(start, template || formatTemplate);
  var endDateStr = format(end, template2 || formatTemplate);

  return "".concat(startDateStr, " \u2013 ").concat(endDateStr);
};

var RangeBox = React.memo(function RangeBox(_ref2)
...
var Schedule = React.memo(function Schedule(_ref)
...
var TimeGridScheduler = React.memo(function TimeGridScheduler(_ref)
...
{var _ref$verticalPrecisio = _ref.verticalPrecision,verticalPrecision = _ref$verticalPrecisio === void 0 ? 30 : _ref$verticalPrecisio,_ref$visualGridVertic = _ref.visualGridVerticalPrecision,visualGridVerticalPrecision = _ref$visualGridVertic === void 0 ? 30 : _ref$visualGridVertic,style = _ref.style,schedule = _ref.schedule,_ref$originDate = _ref.originDate,originDate = _ref$originDate === void 0 ? new Date() : _ref$originDate,classes = _ref.classes,className = _ref.className,onChange = _ref.onChange;
  ...
  var cellInfoToDateRanges = useMemo(
  function () {return (
  ...
  useState(null),_useState2 = _slicedToArray(_useState, 2),pendingCreation = _useState2[0],setPendingCreation = _useState2[1];var _useMemo =
  ...
  function (newDateRange, rangeIndex) {
    ...
    if (!newDateRange) {
      ...
      if (
      isEqual$1(newDateRange[0], newSchedule[rangeIndex][0]) &&
      isEqual$1(newDateRange[1], newSchedule[rangeIndex][1]))
      {
        return;
      }
  ...
  var getDateRangeForVisualGrid = useMemo(
  function () {return (
  ...
  return (
    ...
    React.createElement("div", { style: timelineStickyStyle, "aria-hidden": true, className: classes.timeline },
    ...
    times(7).map(function (i) {return (
        React.createElement("div", { key: i, role: "presentation", className: classes['day-column'] },
